            ...
            <form onSubmit={handleSubmit} className="flex-grow flex flex-col gap-8 relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-1">
                  ...
                  <div className="group pt-2 pb-3 relative">
                    <div className="flex justify-between items-center mb-3">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] group-hover:text-white transition-colors flex items-center gap-2">
                        <div className="w-1 h-1 rounded-full bg-yellow-400 shadow-[0_0_8px_currentColor] opacity-60 group-hover:opacity-100 transition-opacity" />
                        Target Year
                      </label>
                      <input
                        type="number"
                        min={2024}
                        max={2035}
                        value={formData.year || 2028}
                        onChange={(e) => handleInputChange('year', Number(e.target.value))}
                        className="text-xs font-mono text-slate-300 font-bold bg-white/5 px-2 py-0.5 rounded-md border border-white/5 w-20 text-right focus:outline-none focus:border-agri-green/50"
                      />
                    </div>
                  </div>
